    ...
    for<'a, 'b> T: BinRead + BinWrite,
{
    ...
    #[bw(calc = Self::REF_ID_UNIQUE_DEFAULT)]
    ref_id: u64,
    #[brw(args_raw(args))]
    pub value: NdrAlign<T>,
}
...
mod tests {
    ...
    fn test_ptr() {
        ...
        assert_eq!(
            write_result,
            NdrPtr::<u8>::REF_ID_UNIQUE_DEFAULT.to_le_bytes() 
                .to_vec()
                .into_iter()
                .chain(vec![0x00,
                0x00, 0x00, 0x00, // should_align (uninitialized)
                    ]
        );
    }
